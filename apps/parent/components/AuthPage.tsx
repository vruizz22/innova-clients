...
export function AuthPage({ title, mode }: AuthPageProps): JSX.Element {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      setMessage(result.message ?? 'Contraseña restablecida.')
    } catch (requestError) {
      setMessage(
        requestError instanceof Error ? requestError.message : 'Error de autenticación desconocido',
      )
    } finally {
      setLoading(false)
    }
    ...
    <main className="container">
      <section className="card auth-shell">
        <div className="auth-head">
          <a href={runtimeConfig.landingUrl} className="auth-brand">
            SuperProfes
          </a>
          <h1>{title}</h1>
        </div>
            ...
            <>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setResetData((current) => ({ ...current, code: event.target.value }))
                  }
                  type="text"
                  required
                />
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setResetData((current) => ({ ...current, newPassword: event.target.value }))
                  }
                  type="password"
                  required
                />
        ...
        {mode !== 'login' ? (
          <p className="auth-message">
            <a className="auth-link-inline" href="/login">
              Ya tengo cuenta
            </a>
          </p>
        ) : null}
